...
def parse_args():
    ...
    parser.add_argument("--file_dir", help="the train and test set path", type=str,
                        default="/home/leating/datasets/ACL_all/pt")
    parser.add_argument("--train_dir", help="the train dir", type=str,
                        default="/home/leating/datasets/ACL_all/pt/train_100.pt")
    parser.add_argument("--dev_dir", help="the dev dir", type=str,
                        default="/home/leating/datasets/ACL_all/pt/test_100.pt")
    parser.add_argument("--vocab_dir", help="the vocab dir", type=str,
                        default="/home/leating/datasets/ACL_all/pt/vocab_100.pt")
    parser.add_argument("--srcEmbedding_dir", help="the srcembedding dir", type=str)
    parser.add_argument("--tgtEmbedding_dir", help="the tgtEmbedding dir", type=str)  # 这个没有文件
    parser.add_argument("--model_dir", help="the model dir", type=str, default="./trained_model")
    parser.add_argument("--result_dir", help="the result dir", type=str, default="./results/results_all_13")
    parser.add_argument("--pretrain_tgt_embedding", help="the pretrain_tgt_embedding dir", type=str,
                        default="/home/leating/datasets/ACL_wograph/jobvec")
    parser.add_argument("--pretrain_src_embedding", help="the pretrain_src_embedding dir",
                        type=str, default="/home/leating/datasets/ACL_wograph/jobvec")
    ...
    parser.add_argument("--beamSearchSize", type=int, default=1)
    parser.add_argument("--generatealpha", type=float, default=1.4)
    parser.add_argument("--topicGenLamda", type=float, default=0.5)
...
if __name__ == "__main__":
    ...
    print("loading vocabulary/train data/dev data/test data!")
    # load vocb
    data.gVocab = torch.load(data.vocab_dir)
    # load data
    data.train_dataset = torch.load(data.train_dir)
    data.dev_dataset = torch.load(data.dev_dir)

    # load pretrain embedding
    if data.srcEmbedding_dir != "":
        data.pretrain_src_embedding, data.src_embedding_dim, ukn_src_count = build_pretrain_embedding(
            data.pretrain_src_embedding, data.gVocab, "src")
        src_embedding = torch.tensor(data.pretrain_src_embedding)
        print("src unknown words: " + str(ukn_src_count))
    if data.tgtEmbedding_dir != "":
        data.pretrain_tgt_embedding, data.tgt_embedding_dim, ukn_tgt_count = build_pretrain_embedding(
            data.pretrain_tgt_embedding, data.gVocab, "tgt")
    ...
    # optimizer
    # optimizer = torch.optim.Adam(filter(lambda x: x.requires_grad==True, model.parameters()), lr=data.learning_rate)
    optimizer = torch.optim.Adam(model.parameters(), lr=data.learning_rate)
    best_BLEU4 = -10
    best_epoch = -1
    for idx in range(int(data.epoch)):
        ...
        if idx >= -1:
            BLUE_score_1, BLUE_score_2, BLUE_score_3, BLEU4 = evaluate(
                data, model, "dev", data.result_dir + "/dev_output_" + str(idx))
            if BLEU4 > best_BLEU4:
                best_epoch = idx
                time_above = 0
                # print("Exceed previous best BLEU4 score: %f" % dev_BLEU4)
